...
import { CardValueEnum } from '../shared/enums/card-value-enum';
import { CdkDrag, CdkDragDrop, CdkDropList, DragDropModule, transferArrayItem } from '@angular/cdk/drag-drop';
import { CardIdEnum } from '../shared/enums/card-id-enum';
...
export class SolitaireComponent {
  ...
  setDeck() {
    const types: number[] = CardTypeEnum.getTypes();
    const values: number[] = CardValueEnum.getValues();
  ...
  setTypesDecks() {
    this.heartDeck.push(new Card(CardTypeEnum.Heart, 0));
    this.diamondDeck.push(new Card(CardTypeEnum.Diamond, 0));
    this.spadesDeck.push(new Card(CardTypeEnum.Spades, 0));
    this.clubDeck.push(new Card(CardTypeEnum.Club, 0));
  }
  ...
  setGame() {
    this.cardsIds = [...CardIdEnum.getIds()];
    let count = 7;
  ...
  getCardDragged(card: Card, cardIndex: number, column: Card[]): Card | Card[] {
    if(cardIndex == (column.length - 1)) {
      return card;
    }

    return column.slice(cardIndex, column.length - 1);
  }
